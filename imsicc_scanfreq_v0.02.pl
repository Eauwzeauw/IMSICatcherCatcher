...
my $sql = <<'END_SQL';
CREATE TABLE IF NOT EXISTS roguetowers (
	id			UNSINGED INTEGER PRIMARY KEY,
...
#	my @scanner = ();
	my @scanner = `/usr/local/bin/grgsm_scanner -b R-GSM -g 40 -v`;
...
print '3';
			$dbh->do('INSERT INTO roguetowers (frequency, arfcn, lai_mcc, lai_mnc, lai_lac, cellid, neighbourcells, signalstrength) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
				undef,
