...
def processGain():
	...
	for tower in gainTowers:
		...
		cursor.execute("SELECT cellid FROM imsicatchers WHERE cellid = ? AND frequency = ?", (cellid, frequency))
		data = cursor.fetchone()
		cursor.execute("SELECT reselection_offset, temporary_offset FROM towers WHERE cellid = ? AND frequency = ? AND (reselection_offset != 0 OR temporary_offset != 0)", (cellid, frequency))
		gain_data = cursor.fetchone()
		signal_gain = str(gain_data[0]) + ' ' + str(gain_data[1])  
